...
Процедура ЗаполнитьНаборыСвойствОбъекта(Объект, ТипСсылки, НаборыСвойств, СтандартнаяОбработка, КлючНазначения) Экспорт
	...
	Если ТипСсылки = Тип("ДокументСсылка.ПрохождениеОбученияСУО") Тогда
		
		
		Если ТипЗнч(Объект) = ТипСсылки Тогда
			ДанныеПрохождения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				Объект, "ПроведениеОбучения, Сценарий");
		Иначе
			ДанныеПрохождения = Объект;
		КонецЕсли;			
		...
		Строка.Набор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидОбучения, "НаборСвойств");
