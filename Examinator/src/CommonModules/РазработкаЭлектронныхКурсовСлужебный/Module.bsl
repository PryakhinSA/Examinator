...
Процедура ПоместитьФайлыВРесурс(Знач НаборФайлов, СтруктураФайлов, ОписанияФайлов, НастройкиКартинок = Неопределено, ДанныеВидео = Неопределено, ТекстовыеДанныеДляИндексирования = "", ОбновлятьДатуРесурса = Неопределено)
	...
	Попытка
		...
		// Помечаем на удаление файлы, которые не были записаны (используйется в 3.2, в т.ч. при обновлении с 3.0)
...
Функция НастройкиКартинокСлайдовПрезентацииВJSON(НастройкиКартинок) Экспорт
...
Процедура ЗаполнитьМассивВсехСсылокЭлементаСправочника(Знач СсылкаНаЭлемент, ПолныйМассивСсылок, Знач ИсключитьВладельца = Истина, Знач ПроверятьСуществованиеВБазе = Ложь, Знач СчитатьПомеченныеНаУдалениеОтсутствующими = Ложь, ИсключитьГиперссылки = Ложь)
	...
	// Удаляем битые, отсуствующие или ссылки на удаленные элементы
...
Процедура ИнициализироватьМанифестCMI5(ПутьККаталогу) Экспорт
	...
	УстановитьЗначениеМанифестаCMI5(МанифестXDTO.course, "title", ЭлектронноеОбучение.ИмяЭтойИнформационнойБазы(), LangStringТип);	
