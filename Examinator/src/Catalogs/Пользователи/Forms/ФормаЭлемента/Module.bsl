...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Свойства") Тогда
		...
		Если НЕ Объект.Ссылка.Пустая() 
			И Пользователи.ТекущийПользователь() <> Объект.Ссылка
			И Пользователи.ЭтоПолноправныйПользователь(Объект.Ссылка) Тогда
			
			СкрытьДополнительныеСведения = РегистрыСведений.НастройкиПользователейСУО.ПолучитьНастройку(Объект.Ссылка, "СкрытьДополнительныеСведения");
			
			Если СкрытьДополнительныеСведения <> Неопределено И СкрытьДополнительныеСведения = ИСТИНА Тогда
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		...
		КлючЗаписи         = РегистрыСведений.ФотографииПользователейСУО.СоздатьКлючЗаписи(Новый Структура("Пользователь", Объект.Ссылка));
		АдресФотографии_СУ = ПолучитьНавигационнуюСсылку(КлючЗаписи, "Фотография");
		
	КонецЕсли;
	
	РегистрыСведений.СтудентыОО.РазместитьНаФормеДанныеОСтуденте(ЭтотОбъект, Объект.Ссылка, "СтраницаСтудент_СУ");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ДополнительныеСвойства.Вставить("АутентификацияВебКабинета", АутентификацияВебКабинета_СУ);
...
// Проверить обработчик у элемента формы

&НаКлиенте
Процедура АутентификацияВебКабинета_СУПриИзменении(Элемент)
	АутентификацияПриИзменении();
КонецПроцедуры

&НаСервереБезКонтекста
Функция СохранитьФотографиюПользователя(Пользователь, АдресХраненияЗагруженногоФайла = "")
	
	РегистрыСведений.ФотографииПользователейСУО.СохранитьФотографиюПользователя(Пользователь, АдресХраненияЗагруженногоФайла);
	
	// Возвращаем ключ записи
	
	СтруктураКлюча = Новый Структура;
	СтруктураКлюча.Вставить("Пользователь", Пользователь);
	
	КлючЗаписи = РегистрыСведений.ФотографииПользователейСУО.СоздатьКлючЗаписи(СтруктураКлюча);
	
	Возврат ПолучитьНавигационнуюСсылку(КлючЗаписи, "Фотография");
	
КонецФункции

&НаСервереБезКонтекста
Функция УдалитьФотографиюПользователя(Пользователь)
	
	РегистрыСведений.ФотографииПользователейСУО.УдалитьФотографиюПользователя(Пользователь, Истина, Истина);
...
&НаКлиенте
Процедура ИзменитьФото_СУ(Команда)
...
Процедура ПоместитьВыбранныеФайлыВХранилищеВебЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	АдресФотографии_СУ = СохранитьФотографиюПользователя(Объект.Ссылка, Адрес);
	Элементы.Фотография_СУ.Обновить();
	
КонецПроцедуры

&НаКлиенте
Процедура УдалитьФото_СУ(Команда)
	...
	АдресФотографии_СУ = УдалитьФотографиюПользователя(Объект.Ссылка);
	Элементы.Фотография_СУ.Обновить();
...
Процедура ОбщаяНастройкаФормы(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	АутентификацияВебКабинета_СУ = РегистрыСведений.ПользователиВебКабинетаСУ.ПользовательСуществует(, Объект.Ссылка);
	
	Элементы.АутентификацияВебКабинета_СУ.ТолькоПросмотр = НЕ УровеньДоступа.УправлениеСписком;
	
	Элементы.КоманднаяПанельУправлениеФотографией_СУ.Видимость = НЕ ТолькоПросмотр;
...
Процедура ОбновитьИмяДляВхода(Форма, ПриИзмененииНаименования = Ложь)
	...
	Если Элементы.ИмяПереключениеОтметкиНезаполненного.ТекущаяСтраница = Элементы.ИмяБезОтметкиНезаполненного Тогда
		//--! ДОБАВЛЕНО !--//
		// Изменение обусловлено тем, что
		// при снятии флажка "Вход в программу разрешен"
		// и установленном флажке "Аутентификация веб-кабинета"
		// происходит сброс имени пользователя и записать
		// пользователя становится невозможно.
		Возврат;
		//----------------//
...
Процедура УстановитьДоступностьСвойств(Форма)
	...
	Элементы.ПараметрыАутентификации1СПредприятия.Доступность = Форма.ПользовательИБАутентификацияСтандартная ИЛИ Форма.АутентификацияВебКабинета_СУ; //--! ДОБАВЛЕНО ИЛИ...;
...
//--! УСТАНОВИТЬ ПОЛЬЗОВАТЕЛЬСКУЮ ВИДИМОСТЬ !--//

// Для группы ОтображениеРолей значение пользовательской видимости в Ложь
