...
Процедура ИзменитьДействияВФорме(Знач ПользовательИлиГруппа, Знач ДействияВФорме) Экспорт
	...
	Ссылка = ПользовательИлиГруппа;
	
		// Изменения вида интерактивных действий.
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи") Тогда

		// Просмотр контактной информации доступно всем пользователям,
		// Если разрешен доступ к списку пользователей.
		
		Если ПустаяСтрока(ДействияВФорме.КонтактнаяИнформация) Тогда
			ДействияВФорме.КонтактнаяИнформация   = "Просмотр";
		КонецЕсли;
		
		// Редактирование всех свойств доступно не только
		// пользователю с ролью ПолныеПрава, но и пользователю
		// с ролью ДобавлениеИзменениеПользователейИГруппДоступа,
		// если открывается не полноправный пользователь.

		Если НЕ УправлениеДоступом.ЕстьРоль("ПолныеПрава", , Ссылка) И
	  		УправлениеДоступом.ЕстьРоль("ДобавлениеИзменениеПользователей") Тогда
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		// Редактирование всех свойств доступно не только
		// пользователю с ролью ПолныеПрава, но и пользователю
		// с ролью ДобавлениеИзменениеПользователей.

		Если УправлениеДоступом.ЕстьРоль("ДобавлениеИзменениеПользователей") Тогда
...
Функция ПолучитьКраткоеИмяПользователяИБ(Знач ПолноеИмя) Экспорт
	...
	// Примечание: функция скопирована из модуля формы элемента справочника "Пользователи"
