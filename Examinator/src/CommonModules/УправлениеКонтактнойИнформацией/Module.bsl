...
Функция СоздатьПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	//--! ДОБАВЛЕНО !--//
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("СправочникСсылка.Пользователи")
		И ЗначениеЗаполнено(Форма.Объект.Ссылка)
		И ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты
		И НЕ Пользователи.ЭтоПолноправныйПользователь(Пользователи.ТекущийПользователь())
		И Пользователи.ТекущийПользователь() <> Форма.Объект.Ссылка
		И НЕ ПравоДоступа("Добавление", Метаданные.Справочники.Пользователи) Тогда
		
		СкрытьКонтактнуюИнформацию = РегистрыСведений.НастройкиПользователейСУО.ПолучитьНастройку(Форма.Объект.Ссылка, "СкрытьКонтактнуюИнформацию");
		
		Если СкрытьКонтактнуюИнформацию <> Неопределено И СкрытьКонтактнуюИнформацию Тогда
