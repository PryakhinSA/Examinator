...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЭлектронныйРесурс_СУ) Тогда
		Заголовок = "Полнотекстовый поиск в электронных ресурсах";
		Элементы.ОбластиПоискаПредставление.Видимость = Ложь;
	КонецЕсли;
	
	ЕстьПравоИзмененияЭР_СУ = ПравоДоступа("Изменение", Метаданные.Справочники.СводыЗнанийСУЗ);	
	...
	Если Не ПустаяСтрока(Параметры.ПереданнаяСтрокаПоиска) Тогда
		...
		ПриВыполненииПоискаНаСервере(КонтейнерСостояний, СтрокаПоиска,, Параметры.ЭлектронныйРесурс_СУ); //--! ИЗМЕНЕНО !--//
	КонецЕсли;
	...
	РезультатыПоиска = СформироватьРезультатыПоиска(СтрокаПоиска, КонтейнерСостояний, ЕстьПравоИзмененияЭР_СУ);
...
Процедура HTMLТекстПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЭлектронныйРесурс_СУ) Тогда				
		ОткрытьЗначениеПоискаПродолжение(СсылкаHTML.href);					
		Возврат;		
	КонецЕсли;
...
Процедура ПриВыполненииПоиска(НаправлениеПоиска)
	...
	ПриВыполненииПоискаНаСервере(КонтейнерСостояний, СтрокаПоиска, НаправлениеПоиска, Параметры.ЭлектронныйРесурс_СУ);
...
Процедура ПриВыполненииПоискаНаСервере(КонтейнерСостояний, Знач СтрокаПоиска, НаправлениеПоиска = "ПерваяЧасть", ЭлектронныйРесурс)    //--! ИЗМЕНЕНО (добавлено Знач к реквизиту СтрокаПоиска и параметр ЭлектронныйРесурс) !--//
	...
	Если ЗначениеЗаполнено(ЭлектронныйРесурс) Тогда
		...
		ТаблицаЭР = Новый ТаблицаЗначений; // Таблица электронных ресурсов, которые участвуют в поиске
		ТаблицаЭР.Колонки.Добавить("Идентификатор");
		ТаблицаЭР.Колонки.Добавить("Наименование");
		
		СтрокаПоиска = "(" + СтрокаПоиска + ?(ЕстьПравоИзмененияЭР_СУ, "", " НЕ ДоступноПриНаличииПраваИзмененияЭР") + ") И (";
		
		ДанныеЭР =  ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ЭлектронныйРесурс, "ЭтоГруппа, Наименование");
		
		Если ДанныеЭР.ЭтоГруппа Тогда
			...
			Запрос.УстановитьПараметр("Группа", ЭлектронныйРесурс);
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				УникальныйИдентификатор = Строка(ВыборкаДетальныеЗаписи.Ссылка.УникальныйИдентификатор());
				
				СтрокаПоиска = СтрокаПоиска + """" + УникальныйИдентификатор + """ ИЛИ ";
				
				НоваяСтрока = ТаблицаЭР.Добавить();
		...
		Иначе
			...
			УникальныйИдентификатор = Строка(ЭлектронныйРесурс.УникальныйИдентификатор());
			
			СтрокаПоиска = СтрокаПоиска + """" + УникальныйИдентификатор + """";
			
			НоваяСтрока = ТаблицаЭР.Добавить();
			НоваяСтрока.Идентификатор = УникальныйИдентификатор;
			НоваяСтрока.Наименование = ДанныеЭР.Наименование;
	...
	Если ЗначениеЗаполнено(ЭлектронныйРесурс) Тогда
		...
		МассивОтбор.Добавить(Метаданные.РегистрыСведений.ДанныеДляПолнотекстовогоПоискаСУЗ);
		ПараметрыПоиска.ОбластиПоиска = МассивОтбор;
...
&НаКлиентеНаСервереБезКонтекста
Функция СформироватьРезультатыПоиска(СтрокаПоиска, КонтейнерСостояний, ЕстьПравоИзмененияЭР_СУ)  //--! ИЗМЕНЕНО (добавлен реквизит ЕстьПравоИзмененияЭР_СУ) !--//
	
	КодОшибки  = КонтейнерСостояний.КодОшибки;
	
	Если ПустаяСтрока(КодОшибки) Тогда 
		HTMLСтраница = НоваяHTMLСтраницаРезультата(КонтейнерСостояний, ЕстьПравоИзмененияЭР_СУ); //--! ИЗМЕНЕНО (добавлен реквизит ЕстьПравоИзмененияЭР_СУ) !--//
	Иначе 
...
&НаКлиентеНаСервереБезКонтекста
Функция НоваяHTMLСтраницаРезультата(КонтейнерСостояний, ЕстьПравоИзмененияЭР_СУ) //--! ИЗМЕНЕНО (добавлен реквизит ЕстьПравоИзмененияЭР_СУ) !--//
	...
	Для каждого СтрокаРезультатаПоиска Из РезультатыПоиска Цикл 
		...
		Представление = СтрЗаменить(Представление, "Элемент поиска:", "");
		
		СимволНачала = "<br>";
		СимволОкончания = "</div>";
		НачалоПозицииДляУдаления = СтрНайти(ОписаниеHTML, СимволНачала);
		ОкончаниеПозицииДляУдаления = СтрНайти(ОписаниеHTML, СимволОкончания);
		
		Если НачалоПозицииДляУдаления > 0 И ОкончаниеПозицииДляУдаления > 0 Тогда
			ПерваяПоловина = Сред(ОписаниеHTML, 1, НачалоПозицииДляУдаления - 1);
			ПерваяПоловина = СтрЗаменить(ПерваяПоловина, "Данные:", "");			  	
		КонецЕсли;
		
		ШаблонСсылкиРедактировать = "";
		
		Если ЕстьПравоИзмененияЭР_СУ Тогда
			ШаблонСсылкиРедактировать = НСтр("ru = '<br><a href=""%1"">Редактировать</a>'");
			АдресСсылкиРедактировать = Ссылка + "&action=edit";
			ШаблонСсылкиРедактировать = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСсылкиРедактировать, АдресСсылкиРедактировать);			
		КонецЕсли;
		
		ОписаниеHTML = ПерваяПоловина + ШаблонСсылкиРедактировать + СимволОкончания;
...
&НаСервереБезКонтекста
Функция АдресСсылкиВСтруктуру(Знач АдресСсылкиДляОткрытия, Знач РезультатыПоиска)
	
	ЗаписьРегистра = Неопределено;
	Для каждого ЭлементМассива Из РезультатыПоиска Цикл	
		Если СтрНайти(АдресСсылкиДляОткрытия, ЭлементМассива.Ссылка) > 0 Тогда
			ЗаписьРегистра = ЭлементМассива.Значение;
			Прервать;
		КонецЕсли;	
	КонецЦикла;	
	
	Если ЗаписьРегистра = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Результаты поиска не найдены'");
	КонецЕсли;
	
	ДанныеДляОткрытия = Новый Структура;
	ДанныеДляОткрытия.Вставить("ЭлектронныйРесурс", ЗаписьРегистра.ЭлектронныйРесурс);
	ДанныеДляОткрытия.Вставить("ЭлементСодержания", ЗаписьРегистра.ЭлементСодержания);
	ДанныеДляОткрытия.Вставить("СсылкаНаОбъектСУЗ", ЗаписьРегистра.СсылкаНаОбъектСУЗ);
	ДанныеДляОткрытия.Вставить("ЭлементПодборки", ЗаписьРегистра.ЭлементПодборки);
	ДанныеДляОткрытия.Вставить("ЭлементГлоссария", ЗаписьРегистра.ЭлементГлоссария);
	
	Возврат Новый ФиксированнаяСтруктура(ДанныеДляОткрытия);
	
КонецФункции

&НаКлиенте
Процедура ОткрытьЗначениеПоискаПродолжение(Знач АдресСсылкиДляОткрытия) Экспорт
	
	ЭтоСсылкаДляРедактирования = СтрНайти(АдресСсылкиДляОткрытия, "&action=edit") > 0;
	АдресСсылкиДляОткрытия = СТрЗаменить(АдресСсылкиДляОткрытия, "&action=edit", "");
	
	ДанныеДляОткрытия = АдресСсылкиВСтруктуру(АдресСсылкиДляОткрытия, Новый ФиксированныйМассив(КонтейнерСостояний.РезультатыПоиска));
		
	Если ЭтоСсылкаДляРедактирования Тогда
				
		Если ЗначениеЗаполнено(ДанныеДляОткрытия.СсылкаНаОбъектСУЗ) Тогда
			ПоказатьЗначение(,ДанныеДляОткрытия.СсылкаНаОбъектСУЗ);
		Иначе
			ПоказатьЗначение(,ДанныеДляОткрытия.ЭлектронныйРесурс);
		КонецЕсли;		
		
	Иначе
		
		Если ЗначениеЗаполнено(Параметры.ФормаДляОповещения_СУ) И ВладелецФормы <> Неопределено Тогда
			
			Если ЗначениеЗаполнено(ДанныеДляОткрытия.ЭлементПодборки) Тогда
				АдресОбъекта = ОбъектыСУЗПовтИсп.СсылкаНаОбъектСУЗ(ДанныеДляОткрытия.ЭлементПодборки);
			ИначеЕсли ЗначениеЗаполнено(ДанныеДляОткрытия.ЭлементГлоссария) Тогда
				АдресОбъекта = ОбъектыСУЗПовтИсп.СсылкаНаОбъектСУЗ(ДанныеДляОткрытия.ЭлементГлоссария);
			ИначеЕсли ЗначениеЗаполнено(ДанныеДляОткрытия.ЭлементСодержания) Тогда
				АдресОбъекта = ОбъектыСУЗПовтИсп.СсылкаНаОбъектСУЗ(ДанныеДляОткрытия.ЭлементСодержания);		
			ИначеЕсли ЗначениеЗаполнено(ДанныеДляОткрытия.СсылкаНаОбъектСУЗ) Тогда
				АдресОбъекта = ОбъектыСУЗПовтИсп.СсылкаНаОбъектСУЗ(ДанныеДляОткрытия.СсылкаНаОбъектСУЗ);			
			ИначеЕсли ЗначениеЗаполнено(ДанныеДляОткрытия.ЭлектронныйРесурс) Тогда
				АдресОбъекта = ОбъектыСУЗПовтИсп.СсылкаНаОбъектСУЗ(ДанныеДляОткрытия.ЭлектронныйРесурс);
			КонецЕсли;
			
			Оповестить("goToLink_" + Параметры.ФормаДляОповещения_СУ, АдресОбъекта, ЭтотОбъект);			
		...
		Иначе
			...
			ОбъектыСУЗКлиент.ОткрытьОбозреватель(ДанныеДляОткрытия.ЭлектронныйРесурс, АдресОбъекта,, ДанныеДляОткрытия.ЭлементСодержания);			
